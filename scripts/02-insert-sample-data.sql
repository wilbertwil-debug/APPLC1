...
INSERT INTO tickets (title, description, priority, status, created_by, service_station_id, observations)
SELECT 
    'Configuración de acceso VPN',
    'Necesito configurar el acceso VPN para trabajar desde casa. Es mi primer día y no tengo las credenciales.',
    'low',
    'open',
    emp.id,
    ss.id,
    'Nuevo empleado. Coordinar con RRHH para verificar permisos de acceso remoto.'
FROM employees emp, service_stations ss
WHERE emp.name = 'Sofia Herrera'
  AND ss.name = 'Mesa de Ayuda Central'
LIMIT 1;
