...
import { MessageSquare, User, Clock, Eye, EyeOff, Send, Plus, AlertTriangle, Database } from 'lucide-react'
...
export function TicketTrackingDialog({ 
    ...
    try {
      ...
      if (error) {
        ...
        if (error.code === 'PGRST116' || error.message.includes('does not exist')) {
          ...
          setError('La tabla ticket_comments no existe. Ejecuta el script: 05-create-ticket-comments-table-improved.sql')
          return false
        }
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [commentsRes, employeesRes] = await Promise.all([
        supabase
          .from('ticket_comments')
          .select(`
            *,
      ...
      if (commentsRes.error) {
        ...
        if (commentsRes.error.message.includes('does not exist')) {
          setTableExists(false)
          setError('La tabla de comentarios no existe. Ejecuta el script SQL: 05-create-ticket-comments-table-improved.sql')
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!newComment.trim() || !authorId || !tableExists) return
  ...
  const filteredComments = canViewInternal 
    ? (showInternal ? comments : comments.filter(comment => !comment.is_internal))
                ...
                <div className="p-4 space-y-4">
                  {loading ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-400 text-sm">Sé el primero en agregar un comentario para iniciar el seguimiento.</p>
                    </div>
                  ) : (
                    filteredComments.map((comment, index) => (
                      <Card key={comment.id} className={`${comment.is_internal ? 'border-orange-200 bg-orange-50' : 'border-gray-200'} ${index === filteredComments.length - 1 ? 'mb-4' : ''}`}>
                        <CardHeader className="pb-3">
                          <div className="flex items-start justify-between">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {comment.is_internal && (
                                <Badge variant="outline" className="text-orange-600 border-orange-300 bg-orange-50" size="sm">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={isInternal ? 'internal' : 'public'} onValueChange={(value) => setIsInternal(value === 'internal')}>
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
